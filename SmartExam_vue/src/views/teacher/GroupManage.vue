  ...
  <div class="group-manage">
    <div class="search-bar">
      <div class="search-left">
        <el-input v-model="searchText" placeholder="搜索用户组名称">
          <template #append>
    ...
    <el-card class="table-card">
      <div v-if="groupList.length === 0" class="no-data-container">
        <div class="no-data-icon">
          <el-icon :size="60" color="#909399"><Search /></el-icon>
        </div>
        <div class="no-data-text">未搜索到相关用户组</div>
        <div class="no-data-hint">请尝试更换关键词搜索</div>
      </div>
      <el-table v-else :data="groupList" border>
        <el-table-column prop="id" label="ID" width="80" />
        <el-table-column prop="name" label="用户组名称" width="150"/>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button v-if="!isSuperAdmin()" size="default" type="primary" :icon="Edit" @click="handleEdit(scope.row)">
              编辑
            </el-button>
            <el-button size="default" type="danger" :icon="Delete" @click="handleDeleteGroup(scope.row)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
const handleAdd = () => {
  router.push('/teacher/group/edit')
}

const handleEdit = (row) => {
  router.push(`/teacher/group/edit?id=${row.id}`)
}
